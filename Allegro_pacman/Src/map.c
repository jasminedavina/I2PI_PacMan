...
static void draw_K_bean(Map* M, const int row, const int col);
static void draw_S_bean(Map* M, const int row, const int col);
static void draw_L_bean(Map* M, const int row, const int col);
extern int map;
int beans_num = 0;
const char* nthu_map[] = {
	"#####################################",
...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		if (map == 1)
		{
			pFile = fopen("Assets/map_custom.txt", "r");
		}
		else if (map == 2)
		{
	...
	for (int row = 0; row < M->row_num; row++) {
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case 'K':
					draw_K_bean(M, row, col);
					break;

				case 'S':
					draw_S_bean(M, row, col);
					break;

				case 'L':
					draw_L_bean(M, row, col);
					break;
				default:
					break;
			}
...
static void draw_block_index(Map* M, const int row, const int col) {
	...
	dw = block_width / 3;
		...
		al_draw_filled_rectangle(s_x, s_y,
			e_x, e_y, al_map_rgb(144, 120, 196));
...
static void draw_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width/6.0,  al_map_rgb(255, 132, 175));
}

static void draw_power_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(206, 77, 77));
}

static void draw_K_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 224, 152));
}

static void draw_S_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(82, 195, 190));
}

static void draw_L_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(0, 0, 0));
}
