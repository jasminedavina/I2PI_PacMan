...
// You have to modify cage_grid_{x,y} to corressponding flag also.
...
void ghost_draw(Ghost* ghost) {
	...
	*/
	/*
		...
		hint : use ghost->objData.moveCD flag to determine which frame of the animation to draw.
	...
	*/
	...
	if (ghost->status == FLEE) {
		if (timerGhost == 0) {
			...
			if (ghost->objData.moveCD % 64 <= 32) { //if the time frame is within 32
			...
			else if (ghost->objData.moveCD % 64 >= 32) {
		...
		case RIGHT:
			...
			break;
		case UP:
				al_draw_scaled_bitmap(ghost->dead_sprite, 80, 0, 16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			
			break;
		case DOWN:
				al_draw_scaled_bitmap(ghost->dead_sprite, 112, 0, 16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			break;
		default:
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	static Directions proba[5]; // possible movement
	int cnt = 0;
	for (Directions i = 1; i <= 5; i++)
		if (ghost_movable(ghost, M, i, true) && i != 5 - ghost->objData.facing && i != shortestDirection) proba[cnt++] = i;
	Directions ghostrdm;
	if (cnt) {
		ghostrdm = proba[rand() % cnt];
	}
	//so that the ghost runs away from you
	else if (ghost->objData.facing == shortestDirection) ghostrdm = shortestDirection;
	else { ghostrdm = 5 - ghost->objData.facing; }
	ghost_NextMove(ghost, ghostrdm);
